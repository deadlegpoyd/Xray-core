...
func init() {
	...
	// Added "vpb" as an alias for versioned protobuf configs I generate with my build scripts.
	// Added "bpb" as an alias for binary protobuf configs produced by my batch export tool.
	_ = RegisterConfigLoader(&ConfigFormat{
		Name:      "Protobuf",
		Extension: []string{"pb", "protobuf", "proto", "bin", "vpb", "bpb"},
		Loader: func(inputs []*TypedReader) (*Config, error) {
			if len(inputs) == 0 {
				return nil, errors.New("no input provided for protobuf config loader")
			}
